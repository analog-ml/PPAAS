...
class ngspice_env_disc(ngspice_env):
    ...
    def translate_params(self, cur_params):
        """
        Translate the parameter indices to actual parameter values for simulation.
        Example:
        curr_params = [0, 1, 2]
        self.params = [
            [10, 20, 30],  # possible values for param 1
            [1, 2, 3],     # possible values for param 2
            [100, 200, 300] # possible values for param 3
        ]
        The translated parameters would be:
        translated_params = [10, 2, 300]
        """
...
class ngspice_env_cont(ngspice_env):
    ...
    def update_params(self, action):
        ...
        self.cur_params = np.array(
            [
                np.round(self.cur_params[i], precision[i])
                for i in range(len(self.cur_params))
            ]
        )
    ...
    def translate_params(self, cur_params):
        """For continuous parameters, the current parameters are already in the actual parameter values for simulation, so we can directly return them."""
        return cur_params
